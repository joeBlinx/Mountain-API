...
template<size_t n, class ...Ts>
GraphicsPipeline::GraphicsPipeline(Context const &device, SwapChain const &swap_chain, RenderPass const &render_pass, std::array<shader, n> const& shaders,
                                   const std::vector<buffer::vertex> &buffers,
                                   std::vector<vk::DescriptorSetLayout> const& descriptor_layout,
                                   PushConstant<Ts> const& ...push_constant) :
        _device(device){
    static_assert(n >= 2, "shaders arrays must contains at least two entry, one vertex shader and one fragment shader");
...
template<size_t n>
std::vector<vk::PipelineShaderStageCreateInfo> GraphicsPipeline::create_shader_info(std::array<shader, n> const& shaders) {
